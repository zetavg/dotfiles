# Originally from https://github.com/optilude/SublimeTextMisc/blob/master/navigationHistory.py
...
class Location(object):
    ...
    def near(self, other):
        return self.path == other.path and abs(self.line -
                                               other.line) <= LINE_THRESHOLD
...
class History(object):
    ...
    def has_changed(self, location):
        ...
        return self._last_movement is None or not self._last_movement.near(
            location)
...
class NavigationHistoryBack(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        if location:
            window = sublime.active_window()
            window.open_file(
                "%s:%d:%d" % (location.path, location.line, location.col),
